...
chrome.contextMenus.onClicked.addListener((info, tab) => {
  ...
  switch (info.menuItemId) {
    ...
    case "sendAndTranslateToQwen":
      const textWithTranslatePrompt = "Переведи на русский язык\n\n" + selectedText;
    ...
    case "sendToGemini":
      ...
      break;
    case "sendToCopilot":
      openAndInsertText("https://copilot.microsoft.com/*", "https://copilot.microsoft.com/", selectedText, insertTextToCopilot);
      break;
  }
});
...
// Функция для вставки текста в Grok (универсальная)
...
// Функция для вставки текста в Microsoft Copilot
function insertTextToCopilot(text) {
  const waitForInput = setInterval(() => {
    let inputElement = null;
    
    // Microsoft Copilot может использовать разные элементы
    inputElement = document.querySelector('textarea[aria-label*="Ask me anything"]');
    if (!inputElement) {
      inputElement = document.querySelector('textarea');
    }
    if (!inputElement) {
    ...
    if (inputElement) {
      ...
      if (inputElement.tagName === 'TEXTAREA' || inputElement.tagName === 'INPUT') {
        ...
        // Для Copilot используем execCommand (проверенный метод)
        document.execCommand('insertText', false, text);
